...
public class FeedClass {
	...
	// Atributos
	public static double valor = 0.0;	// Atributo Static para manipular em outra Classe
	...
	static void Feed() {	// Menu de Opções de Categorias
		...
		case 1: FeedClass.AventuraAcao(); break;
	...
	static void removerItem() {
		...
		String textoJogos = "";
		for(int i=0;i<jogosCarrinho.size();i++) {
			textoJogos += (i + 1) + ") " +jogosCarrinho.get(i).getNomeJogo() +"\n Preco: "+jogosCarrinho.get(i).formatarMoeda()+"\n";
		}
		...
		valor -= jogosCarrinho.get(resul).getPrecoUni();
		jogosCarrinho.remove(resul);
		if (jogosCarrinho.size() == 0) {
			valor = 0;
		}
	...
	public static void setValor(double valor) { FeedClass.valor = valor; }
